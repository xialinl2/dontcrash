...
$sql = "INSERT INTO ".
		"`accidents`(`CRASH_DATE`,`POSTED_SPEED_LIMIT`,`TRAFFIC_CONTROL_DEVICE`,`DEVICE_CONDITION`,`WID`,`LID`,`FIRST_CRASH_TYPE`, ".
		"`TRAFFICWAY_TYPE`,`LANE_CNT`,`ALIGNMENT`,`ROADWAY_SURFACE_COND`,`CRASH_TYPE`,`HIT_AND_RUN_I`,`DAMAGE`,`DATE_POLICE_NOTIFIED`, ".
		"`PRIM_CONTRIBUTORY_CAUSE`,`SEC_CONTRIBUTORY_CAUSE`,`STREET_NAME`,`MOST_SEVERE_INJURY`,`INJURIES_TOTAL`,`INJURIES_FATAL`,`CRASH_HOUR`, ".
		"`CRASH_DAY_OF_WEEK`,`CRASH_MONTH`,`LOCATION_ID`, `USER`) VALUES ( ";
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	if(!empty($_POST['policenotified'])){
		...
		$no_of_params = $no_of_params + 1;
		$sql = $sql.", '".$policenotified." 00:00:00'" ;
	} else {
		$sql = $sql.", NULL" ;
	}
...
header('Location: report.php?newrdno='.$newrdno);
exit();
$result->free();
$conn->close();
