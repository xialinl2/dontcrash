...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		if($i==1){$location_select=$location_select."<table><tbody><tr>"}
		$lid = $row["location_id"];
		$area = $row["community_area"];
		$community = $row["community_side"];
		$location_select = $location_select.'<td><input type="checkbox" name="location[]" value="'.$lid.'" id="'.$lid.'"><label for="'.$lid.'">'.$area.'    -    '.$community.'</label></td><td>&nbsp;</td>';
		if($i%4 == 0){$location_select=$location_select."</tr><tr>"}
		if($i==78){$location_select=$location_select."</tr></tbody></table>"}
		$i++;
	}
	$result->free();
...
?>					
					  ...
					  <label class="heading">Select Community:</label>
